       ...
       <form method="POST" action="/appmaka/SubOrder/submit" onSubmit="return mycheck()" name="form1" id="form1" >
        ...
        <div class="line_div"> <span class="text">付款方式</span>
          <div class="input_div">
            <label>
              ...
              货到付款 
			  <input  type="radio" name="payment" class="payment" value="支付宝"/>
              支付宝 
			  </label>
        ...
        <div class="line_div_submit">
			<input type="hidden" value="<?php echo (session('useradmin')); ?>" name="operator" />
			 <input type="hidden" value="官网正品玛卡" name="subject" class="text">
            <input type="hidden" value="maka" name="system" class="text">
            <input type="hidden" value="appmaka" name="runUrl" class="text">
...
$(function(){
		$("input[name=payment]").on("change",function(){
				...
				if(payType=="货到付款"){
					$("form[name=form1]").attr("action","/appmaka/SubOrder/Submit");
				}else{
					$("form[name=form1]").attr("action","/appmaka/Pay/doalipay");
				}
